...
Given the robot's current angle, this function calculates the angle to the goal. cur\-\_\-angle, goal\-\_\-x, goal\-\_\-y, px, and py are class fields but are also passed as parameters. 
...
\hline
{\em cur\-\_\-angle} & The robot's current angle, in reference to the co-\/ordinate system \\
\hline
{\em px} & The robot's initial x position \\
\hline
{\em py} & The robot's initial y position \\
\hline
...
Keeps the robot moving by changing linear\-\_\-x ad angular\-\_\-z. 
...
\hline
{\em cur\-\_\-angle} & The robot's current facing, in reference to the co-\/ordinate system. \\
\hline
{\em goal\-\_\-angle} & The angle that the robot must face in order to reach the goal. \\
\hline
...
Causes the robot to move until the goal is reached. When the goal is reached, the next checkpoint becomes the goal or if the list of checkpoints is exhausted, the robot returns to its initial position (the first checkpoint) 
...
\hyperlink{classResident}{Resident} boredom \hypertarget{classAssistant_ac3c8eb895beffec2e70b25184c7fcae9}{\index{Assistant@{Assistant}!checkpoints@{checkpoints}}
...
test\-The x position of the robot's goal \hypertarget{classAssistant_a1b6ce5d7c3124140be8d351c847fe91c}{\index{Assistant@{Assistant}!goal\-\_\-y@{goal\-\_\-y}}
